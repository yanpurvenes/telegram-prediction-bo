...
async def setup_channel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not channel_id.startswith('@') and not channel_id.startswith('-100'):
        await update.message.reply_text("Неверный формат ID канала. Используйте @username или -100xxxxxxxxxx")
        return
    ...
    try:
        ...
        if not chat:
            await update.message.reply_text("Не удалось получить доступ к каналу. Убедитесь, что бот добавлен в канал как администратор.")
            return
...
def main() -> None:
    ...
    application.job_queue.run_once(
        lambda context: asyncio.create_task(send_daily_predictions(context)),
        when=0
    )
